...
void loop() {
  ...
  if (BUTTONLOW) { // timer select and sleep
    seconds = 0;
    minutes = 0;
    hours = 0;
    ...
    while (BUTTONLOW) { // enter sleep if button is pressed for 1 sec.
      if ((interruptTimer - timer2) > 995) {
        clearScreen();
...
void displayInfo(void) {
  if ((interruptTimer - timer) > 200) {
    ...
    if (timerAlarm == minutes && timerAlarm != 0) {
      while (BUTTONHIGH) {
    ...
    drawSprite(18, 1, tempSimbol, false); // display temp

    if (lastMinute != minutes) { // display seconds, minutes and hours.
      lastMinute = minutes;
...
void buttonDebounce(void) {
  timer = interruptTimer;
  while (BUTTONLOW || (interruptTimer - timer) < 200); // super simple button debounce
}
...
ISR(TCB0_INT_vect) {// timmer
  ...
  if (interruptTimer++, interruptTimer >= 1000) {
    interruptTimer = 0;
