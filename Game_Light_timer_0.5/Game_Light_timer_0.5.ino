...
void setup() {
  ...
  OledDisplayMode = eeprom_read_byte((uint8_t*)127);
  oled.ssd1306_send_command(OledDisplayMode);
}
...
void displayInfo(void) {
  if (lastSecond != seconds) {
    lastSecond = seconds;
    int8_t actualTemp = 0;
    int8_t voltageReading = map(readSupplyVoltage(), 2500, 3800, 0, 117);

    if (timerAlarm != 0) { // draw sandClock
      oled.drawLine(8, 3, 119, 0b00000000);
      totalSeconds++;
      drawSandClock ();
    }

    if ((timerAlarm == minutes && timerAlarm != 0) || voltageReading < 1) { // if the alarm is in on state the led is turn on, if the button is pressed it goes to sleep.
      clearScreen();
    ...
    if (lastMinute != minutes) {
      ...
      if (useCelsius) {
        drawSprite(0, 1, tempSimbolC, false);
        actualTemp = (readTemp() - 283);
      } else {
        drawSprite(0, 1, tempSimbolF, false);
        actualTemp = (readTemp() - 265);
      }
      drawDecimal (10, actualTemp); //0°C = 273.15K - 0°F = 255.37
    }

    if (actualTemp < 0) oled.drawLine (4, 2, 5, 0b00000100); // draws screen colons
    drawDecimal (34, hours);
    oled.drawLine (54, 2, 1, 0b00010001); // draws screen colons
    drawDecimal (58, minutes);
    oled.drawLine (78, 2, 1, 0b00010001);
...
void options (void) { // options menu
  int8_t option = 0;
  uint16_t optionTimer = 0;
  uint16_t lastInterruptTimer = interruptTimer;
  ...
  drawSprite(80, 1, screen, false);
  buttonDebounce();

  while (setOptions) {

    if (lastInterruptTimer != interruptTimer) { // counts time
      lastInterruptTimer = interruptTimer;
      optionTimer++; // every odd millisecond option timer increments
    }
    ...
    switch (option) { // option select menu.
      ...
      case 2:
        ...
        if (optionTimer > 2000) {
          ...
          if (OledDisplayMode == 0xA6) {
            OledDisplayMode = 0xA7;
            oled.ssd1306_send_command(0xA7);
            eeprom_write_byte((uint8_t*)127, 0xA7);
          } else {
            OledDisplayMode = 0xA6;
            oled.ssd1306_send_command(0xA6);
            eeprom_write_byte((uint8_t*)127, 0xA6);
          }
          setOptions = false;
        }
        break;
